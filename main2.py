...
class PokeToLLM(Star):
    ...
    async def on_poke(self, event: AstrMessageEvent):
        ...
        # ===== 获取用户真实昵称（新增部分）=====
        username = sender_id_str  # 默认回退为QQ号
        try:
            ...
            if bot:
                if group_id:
                    ...
                    if hasattr(bot, 'get_group_member_info'):
                        ...
                        if member_info:
                            username = member_info.get('card') or member_info.get('nickname') or username
                else:
                    # 私聊：获取陌生人信息
                    if hasattr(bot, 'get_stranger_info'):
                        stranger_info = await bot.get_stranger_info(user_id=int(sender_id))
                        if stranger_info:
                            username = stranger_info.get('nickname') or username
        except Exception as e:
            logger.debug(f"[PokeToLLM] 获取用户信息失败，使用默认QQ号: {e}")
